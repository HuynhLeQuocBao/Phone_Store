...
public class billDetailsDAL {
		...
		if (dc.openConnection()) {
			try {
				...
				while (resultSet.next()) {
					BillDetails std = new BillDetails(resultSet.getString("BILL_DATE"), resultSet.getInt("BILL_DETAILS_CODE"),
							resultSet.getInt("CUSTOMER_CODE"),
	...
	public boolean confirmOrder(BillDetails p) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sql = "update bill_details set CONFIRM=?, EMPLOYEE_CODE=? where BILL_DETAILS_CODE=? AND BILL_DATE='"
						+ p.getBillDate() + "'";
				PreparedStatement statement = dc.connection.prepareCall(sql);
	...
	public boolean hasBillDetailsCode(String billDate, int billDetailsCode) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sql = "select * from bill_details where BILL_DATE= '" + billDate + "' and BILL_DETAILS_CODE="
						+ billDetailsCode;
				Statement statement = dc.connection.createStatement();
	...
	// statistic
	public List<statis> statistic(List<Supplier> listSupllier, List<Product> listProduct, String monthNow)
