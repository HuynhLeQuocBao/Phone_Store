...
public class enterCouponDAL {
	...
	public int getTotal(String code) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sqlGetTotal = "select SUM(enter_coupon_details.TOTAL) as TOTAL from enter_coupon_details where enter_coupon_details.COUPON_DATE= '"
						+ code + "'";
				Statement statement = dc.connection.createStatement();
	...
	public int getAmount(String code) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sqlGetTotal = "select SUM(enter_coupon_details.AMOUNT) as AMOUNT from enter_coupon_details where enter_coupon_details.COUPON_DATE= '"
						+ code + "' ";
				Statement statement = dc.connection.createStatement();
