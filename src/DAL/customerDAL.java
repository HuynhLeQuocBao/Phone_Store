		...
		if (dc.openConnection()) {
			try {
				...
				if (statementProduct.executeUpdate() >= 1) {
					...
					// lấy billDetailsCode
					String getBillDetailsCodeSql = "select MAX(BILL_DETAILS_CODE) as BILL_DETAILS_CODE from bill_details where BILL_DATE='"
							+ billDate + "'";
					...
					BillDetails bd = new BillDetails(billDate, billDetailsCode, customerCode, productCode, employeeCode, amount,
							price, total, 0);
