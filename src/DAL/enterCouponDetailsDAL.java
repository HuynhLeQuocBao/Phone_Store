...
public class enterCouponDetailsDAL {
		...
		if (dc.openConnection()) {
			try {
				...
				while (resultSet.next()) {
					EnterCouponDetails std = new EnterCouponDetails(resultSet.getString("COUPON_DATE"),
							resultSet.getString("COUPON_CODE_DETAILS"),
	...
	public boolean hasEnterCouponDetails(String couponDate, String couponCodeDetails) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sql = "select * from enter_coupon_details where COUPON_DATE='" + couponDate
						+ "' and COUPON_CODE_DETAILS=" + couponCodeDetails;
				Statement statement = dc.connection.createStatement();
