...
public class billDAL {
	...
	public int getTotal(String date) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sqlGetTotal = "select SUM(bill_details.TOTAL) as TOTAL from bill_details where bill_details.BILL_DATE= '"
						+ date + "' ";
				Statement statement = dc.connection.createStatement();
	...
	public int getAmount(String date) {
		...
		if (dc.openConnection()) {
			try {
				// query
				String sqlGetTotal = "select SUM(bill_details.AMOUNT) as AMOUNT from bill_details where bill_details.BILL_DATE= '"
						+ date + "' ";
				Statement statement = dc.connection.createStatement();
