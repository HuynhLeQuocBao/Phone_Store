...
public class billDetailsGUI extends JFrame {
	...
	private void btnAddActionPerformed(ActionEvent e) {
		if (!tfBillDate.getText().trim().equals("") && !tfBillDetailsCode.getText().trim().equals("")
				&& !tfAmount.getText().trim().equals("") && !tfPrice.getText().trim().equals("")) {
			try {
				...
				String productCode = String
						.valueOf(dh.changeNameToCode("product", "PRODUCT_CODE", productName, "PRODUCT_NAME"));
				...
				BillDetails p = new BillDetails(billDate, billDetailsCode, customerCode, productCode, employeeCode, amount,
						price, total, 1);
			...
			if (billDetailsList.size() == 0) {
				...
				while (i < billDetailsList.size()) {
					...
					model.addRow(new Object[] {
							model.getRowCount() + 1, p.getBillDate(), p.getBillDetailsCode(), p.getCustomerCode(), p.getProductCode(),
							p.getEmployeeCode(), p.getAmount(), Tester.format(p.getPrice()), Tester.format(p.getTotal())
					});
					i++;
				}
			}
	...
	private void btnEditActionPerformed(ActionEvent e) {
		try {
			...
			if (index >= 0 && !tfBillDate.getText().equals("") && !tfBillDetailsCode.getText().trim().equals("")
					&& !tfAmount.getText().equals("") && !tfPrice.getText().equals("")) {
	...
	private void btnRemoveActionPerformed(ActionEvent e) {
		...
		if (selectedIndex >= 0 && !tfBillDate.getText().equals("")) {
			...
			int option = JOptionPane.showConfirmDialog(null, "Bạn có muốn xóa phiếu xuất này không?", "Question",
					JOptionPane.YES_NO_OPTION);

			if (option == JOptionPane.YES_OPTION) {
				int sure = JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn xóa phiếu xuất này không?", "Question",
						JOptionPane.YES_NO_OPTION);
