...
public class accountGUI extends JFrame {
	...
	public void initComponent() {
		...
		if (employeeInterfaceGUI.employeeCode != -1) {
			// bỏ tài khoản nhân viên và tài khoản admin ra khỏi mảng hiển thị chỉ hiện tài
			// khoản khách hàng
			...
			if (accountList.size() == 0) {
				...
				while (i < accountList.size()) {
					...
					model.addRow(new Object[] {
							model.getRowCount() + 1, p.getId(), p.getAccountName(), p.getPassword(), p.getUserName(), p.getAddress(),
							p.getBirthday(), p.getNumberPhone(), p.getPermission()
					});
					i++;
				}
			}
	...
	private void btnRemoveActionPerformed(ActionEvent e) {
		...
		if (selectedIndex >= 0 && !tfAccountName.getText().equals("")) {
			...
			if (option == JOptionPane.YES_OPTION) {
				int sure = JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn xóa tài khoản này không?", "Question",
						JOptionPane.YES_NO_OPTION);
